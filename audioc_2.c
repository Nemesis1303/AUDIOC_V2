...
#define AUDIO_BLOCK_TAKEN 4
#define SILENCE_AUDIO_BLOCK_INSERTED 5
#define PACKET_LOST 6
#define SELECT_TIMER_EXPIRES 7
#define RTCP_PACKET_ARRIVES 8
#define RTCP_PACKET_DEPARTURES 9

#define ZERO_8 128          // U8 range is {0,255}, hence its SILENCE (No sound) is defined as the middle point (128) of the range values
#define ZERO_16 0           // L16 range is {-32768,32767}, similarly as in U8, the middle point is now 0

/*If silence values are defined as the middle point of the range values in each format, values with some noise will be defined as values around the silence values.*/
#define NOISE_8_DOWN 127
#define NOISE_8_UP 129
#define NOISE_16_DOWN -1
#define NOISE_16_UP 1

/* GLOBAL VARIABLES */
/*Pointers*/
void * circularBuffer = NULL;
void * packet_send = NULL;
void * packet_receive = NULL;
void * play_from_buffer = NULL;
rtcp_t * bye_packet_send = NULL;
rtcp_t * bye_packet_receive = NULL;
...
void * audioData_packet_send = NULL;
void * audioData_packet_receive = NULL;
...
/*Constant some noise arrays*/
uint8_t* SOME_NOISE_8 [] = {(uint8_t*) NOISE_8_DOWN, (uint8_t*)NOISE_8_UP, (uint8_t*)NOISE_8_DOWN, (uint8_t*)NOISE_8_UP, (uint8_t*)NOISE_8_DOWN, (uint8_t*)NOISE_8_UP, (uint8_t*)NOISE_8_DOWN, (uint8_t*)NOISE_8_UP}; 
int16_t* SOME_NOISE_16 [] = {(int16_t*) NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP, (int16_t*)NOISE_16_DOWN, (int16_t*)NOISE_16_UP}; 
...
struct sockaddr_in remToSendSAddr_RTCP; // Multicast address RTCP (port RTP+1)
struct sockaddr_in remToRecvSAddr;      // To receive
struct ip_mreq mreq;                    // For multicast configuration
socklen_t sockAddrInLength;             // For recvfrom  

bool first_packet = true;
bool bye_received = false;
uint32_t member_ssrc = 0; // This should be an array containing all the members that are in the communication. Though, the number of peers that we are going to have is 1.
...
void do_free()
{
   ...
   if (packet_send) free(packet_send);
   if (packet_receive) free(packet_receive);
   if (play_from_buffer) free (play_from_buffer);
   if (silence_package) free(silence_package);
   if (bye_packet_send) free(bye_packet_send);
   if (bye_packet_receive) free(bye_packet_receive);
}
 ...
 *          RTCP_PACKET_ARRIVES:            Every time a RTCP arrives.
 *          RTCP_PACKET_DEPARTURES:         Every time a RTCP departures.
 */
void do_verbose(int type)
{
    switch (type)
    {
        ...
        case SELECT_TIMER_EXPIRES:
            printf ("t");
            break;
        case RTCP_PACKET_ARRIVES:
            printf ("<");
            break;
        case RTCP_PACKET_DEPARTURES:
            printf (">");
            break;
        default:
            break;
...
/*
 * Function: createByePacket 
 * -----------------------------
 *      Creates a Bye Packet.
 */
void createByePacket(rtcp_t *byePacket, uint32_t ssrc)
{
    (*byePacket).common.version = 2;
    (*byePacket).common.p = 0;
    (*byePacket).common.count = 0; // Since only the BYE packet has been implemented, this value is set to 0
    (*byePacket).common.pt = RTCP_BYE;
    // Since the variable 'x' in the RTP header is set to 0, the variable length in the RTCP header
    // does not need to be appended   
    (*byePacket).r.bye.src[1] = ssrc; // For our specific application, the number of peers that we are gonna have is always one
}
...
{   
    ...
    {
        int result;
        result = sendto(socketDesc, bye_packet_send, (sizeof(rtcp_t) ) , 0 , (struct sockaddr *) &remToSendSAddr_RTCP, sizeof(remToSendSAddr));
        ...
        {   
            if(verbose) do_verbose(RTCP_PACKET_DEPARTURES);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    /*************************/
    /* Configures socket RTCP*/
    /*************************/
    /* Preparing bind for multicast */
    bzero(&remToSendSAddr_RTCP, sizeof(remToSendSAddr_RTCP));
    remToSendSAddr_RTCP.sin_family = AF_INET;
    remToSendSAddr_RTCP.sin_port = htons(port);
    remToSendSAddr_RTCP.sin_addr = multicastIp;
    sockAddrInLength = sizeof (struct sockaddr_in); 

    /* Creates socket */
    if ((socketDescRTCP = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        printf("socket error\n");
        exit(EXIT_FAILURE);
    }

    /* configure SO_REUSEADDR, multiple instances can bind to the same multicast address/port */
    if (setsockopt(socketDescRTCP, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
        printf("setsockopt(SO_REUSEADDR) failed");
        exit(EXIT_FAILURE);
    }

    if (bind(socketDescRTCP, (struct sockaddr *)&remToSendSAddr_RTCP, sizeof(struct sockaddr_in)) < 0) {
        printf("bind error\n");
        exit(EXIT_FAILURE);
    }
    ...
    mreq.imr_multiaddr = multicastIp;
    mreq.imr_interface.s_addr = htonl(INADDR_ANY);
    if (setsockopt(socketDescRTCP, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
        printf("setsockopt error");
        exit(EXIT_FAILURE);
    }

    /* Do not receive packets sent to the mcast address by this process */
    setsockopt(socketDescRTCP, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(unsigned char));
    /*************************/
    /* End socket configuration */
    /*************************/
    ...
    /**********************************************/
    packet_send = malloc(sizeof(rtp_hdr_t) + requestedFragmentSize);
    if (packet_send == NULL)
    {
        printf("Memory for packet send could not be reserved.");
    }

    packet_receive = malloc(sizeof(rtp_hdr_t) + requestedFragmentSize);
    if (packet_receive == NULL)
    {
        printf("Memory for packet receive could not be reserved.");
    }

    /************************************************/
    /*DEFINING RTP HEADERS FOR SENDING AND RECEIVING*/
    /************************************************/
    // We reinterpret packet_send and packet_receive as rtp_hdr_t memory blocks 
    rtp_hdr_t * RTPHeader_packet_send;
    RTPHeader_packet_send = (rtp_hdr_t *) packet_send; 

    rtp_hdr_t * RTPHeader_receive;
    RTPHeader_receive = (rtp_hdr_t *) packet_receive;   
    ...
    {
        printf("Memory for play from buffer could not be reserved.");
    }
    ...
    /*DEFINE BYE PACKET*/
    /*********************************/
    bye_packet_send = malloc(sizeof(rtcp_t));
    if (bye_packet_send == NULL)
    {
        printf("Memory for bye packet could not be reserved.");
    }

    bye_packet_receive = malloc(sizeof(rtcp_t));
    if (bye_packet_receive == NULL)
    {
        printf("Memory for bye packet could not be reserved.");
    }

    createByePacket(bye_packet_send, ssrc);
    ...
    {   
        ...
        {  
            ...
            {   
                audioData_packet_send = (RTPHeader_packet_send + 1);
                bytesRead = read(sndCardDesc, audioData_packet_send, requestedFragmentSize);
                ...
                {   
                    ...
                    createRTPheader(RTPHeader_packet_send, payload, sequenceNr_send, timestamp_send, ssrc);

                    // Data is sent independently of if silences are contained or not
                    dataSent = sendto(socketDesc, packet_send, (sizeof(rtp_hdr_t) + requestedFragmentSize) , 0 , (struct sockaddr *) &remToSendSAddr, sizeof(remToSendSAddr));
            ...
            {
                dataReceived = recvfrom(socketDesc, packet_receive, (sizeof(rtp_hdr_t) + requestedFragmentSize) , 0 , (struct sockaddr *) &remToRecvSAddr, &sockAddrInLength);
                ...
                {   
                    if(first_packet) 
                    {
                        member_ssrc = ntohl(RTPHeader_receive -> ssrc);
                        first_packet = false;
                    }
                    ...
                    audioData_packet_receive = (RTPHeader_receive+1);
                    if ((pointerWriteInBuffer = cbuf_pointer_to_write(circularBuffer)) != NULL)
                    {   
                        memcpy(pointerWriteInBuffer, audioData_packet_receive, requestedFragmentSize);
                        ++nrPacketsInBuffer;
                    }
                    else
                    {
            ...
            if (FD_ISSET(socketDescRTCP, &readSet) == 1)
            {
                dataReceived = recvfrom(socketDescRTCP, bye_packet_receive, (sizeof(rtcp_t)) , 0 , (struct sockaddr *) &remToRecvSAddr, &sockAddrInLength);
                ...
                {   
                    if(verbose) do_verbose(RTCP_PACKET_ARRIVES);
                    bye_received = true;
                    member_ssrc = 0;
                    signalHandler(SIGINT);                    
                }   
            } // end socket RTCP descriptor reading ready
        } // end res != 0
